...
def gaussian_smooth(input_, tau):
    ...
    for i, id_ in enumerate(ids):
        ...
        output_.extend([
            [t[j, 0], id_, xx[j], yy[j], ww[j], hh[j], tracks[j, 6], tracks[j, 7], -1, -1] for j in range(len(t))
        ])
    return output_


def gsi(mot_results_folder=Path('/media/samar/HDD1T/Deep-EIoU/Deep-EIoU/tools/track_vis'), interval=20, tau=10):
    tracking_results_files = mot_results_folder.glob('SNMOT*.txt')
    for p in tracking_results_files:
        ...
        if tracking_results.size != 0:
            ...
            np.savetxt(p, gsi, fmt='%d %d %d %d %d %d %d %d %d %d',delimiter=',')
        else:
