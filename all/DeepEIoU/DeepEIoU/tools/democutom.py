...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        _, ratio = preproc(img, self.test_size, self.rgb_means, self.std)
        ...
        #with torch.no_grad():
            #timer.tic()
        outputs = self.model.predict(img,conf=self.confthre,iou=self.nmsthre,imgsz=self.test_size)
            #outputs = self.model
...
def imageflow_demo(predictor, extractor, vis_folder, current_time, args):
    ...
    #os.makedirs(save_folder, exist_ok=True) #remove
    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                det = outputs[0].boxes.xyxy.cpu().numpy()
                conf = outputs[0].boxes.conf.cpu().numpy()
                # print(conf)
                # print(det)
                # print(type(det))
                det = np.hstack([det,np.expand_dims(conf,axis=1)])
                # print(det)
            ...
            #if args.save_result:
            #    vid_writer.write(online_im) #remove
            ch = cv2.waitKey(1)
...
def main(exp, args):
    ...
    #vis_folder = osp.join(output_dir, "gt")
